describe('createAccount', function () {
    ...
    it('test require fields', function () {
        cy.fixture('/create.account.json').then(data => {
            ...
            cy.get('#SemesterId').select(data.semesterId);
            cy.get('#FieldOfStudyIds').select(data.fieldOfStudyIds);
            cy.get('.btn').contains('Zarejestruj').should('be.visible').click();

            cy.get('#Email-error').contains('Pole wymagane').should('be.visible');

            cy.get('#Surname-error').contains('Pole wymagane').should('be.visible');
        })
    });
});